...
class Game:
    def __init__(
        self, level="hard", headless=False, mute=False, show_flags=False, log=print
    ):
        self.init = True
        self.stale = False
        ...
        match level:
            ...
            case "hard":
                ...
                self.driver.find_element(By.XPATH, "//*[@data-difficulty='HARD']").click()
            case "easy":
                self.cols = 10
                self.rows = 8
                self.flags = 10
                ...
                self.driver.find_element(By.XPATH, "//*[@data-difficulty='EASY']").click()



        self.initial = (self.cols // 2, self.rows // 2)

        self.board = {(x, y): "?" for x in range(self.cols) for y in range(self.rows)}
    ...
    def save_screenshot(self, path):
        Image.open(BytesIO(self.canvas.screenshot_as_png)).save(path)
    ...
    def get_board_screenshot(self):
        self.reload_canvas_dims()
        return np.array(Image.open(BytesIO(self.canvas.screenshot_as_png)))
    ...
    def update(self):
        ...
        for x in range(self.cols):
            for y in range(self.rows):
                ...
                if n in lut and lut[n] != "":
                    self.board[(x, y)] = self.lookup[self.level][n]
                else:
                    if not self.init:
                        # Settle board
                        time.sleep(1)
        ...
        for pos in prev:
            if prev[pos] != self.board[pos]:
                if not self.show_flags and prev[pos] == "F" and self.board[pos] == "?":
                    continue
                if prev[pos] == "?":
                    continue
                if prev[pos] == "#" and self.board[pos].isdigit():
                    continue
                else:
                    msg = f"GAME CORRUPTED!\nExpecting '{prev[pos]}' but found '{self.board[pos]}' at {pos}"
                    print(msg)
                    self.log(msg)
                    breakpoint()
                    raise SystemExit()
    ...
    def write_lookup(self):
        with open("./lookup.json", "w", newline="\n") as f:
